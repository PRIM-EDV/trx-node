...
import { fromEntityType } from "src/infrastructure/mapper/entity.mapper.service";
...
export class TrackerApiService {
    ...
    private handleRequest({ msgId, request }: { msgId: string, request: Request }) {
        if (request.setEntity) {
            ...
            const entityType = fromEntityType(request.setEntity.entity.type);
            this.mapEntity.updatePosition(entity.id, entity.position, entityType);
        }
    }
}